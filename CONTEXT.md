...
**Trip**
A span of travel for one Vehicle, stopped either manually in the web app or automatically by the poll trigger once the vehicle has stayed parked long enough after its last DriveSegment closed (see Segment inference rules), and either manually started there or auto-started by the poll trigger when it finds a Vehicle with no active trip sitting in `shift_state == D` (a single sample is enough — see Segment inference rules). A Trip is the top-level container: it owns DriveSegments, ChargeSessions, and the RouteLog for the time window between its start and stop. A Vehicle may have many Trips; Trips do not overlap for the same Vehicle.
...
- **Trip auto-stop** fires once every DriveSegment on the Trip is closed (none still open/driving) and the last one closed ≥20 minutes ago, and the vehicle's latest sample reads `shift_state == P` — i.e. ~30 minutes of continuous parked time after the vehicle actually stopped moving, in total. The extra 20 minutes on top of the DriveSegment's own close grace gives the user a window to keep the Trip open (e.g. a quick errand) before it closes on its own. A Trip with no closed DriveSegment yet (e.g. the very first poll right after auto-start) is never auto-stopped.
